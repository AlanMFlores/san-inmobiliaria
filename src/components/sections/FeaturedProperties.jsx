  ...
  return (
    <section className='mt-10 lg:mt-20 mb-10 lg:mb-20 mx-8 lg:mx-32'>
        ...
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 justify-center mb-8 lg:mb-16'>
          {
            properties.map(({title, slug, zone, ambience, metres, price, cover, operation}) => (
              <PropertyCard title={title} slug={slug} zone={zone} cover={cover} ambience={ambience} metres={metres} price={price} operation={operation} key={title}/>
            ))
          }
        </div>
